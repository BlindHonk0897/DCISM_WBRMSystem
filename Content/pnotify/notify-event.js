$(function () {
    ...
    $('#pnotify-solid-styled-right').on('click', function () {
        new PNotify({
            title: 'Right icon',
            text: 'Check me out! I\'m a notice.',
            addclass: 'alert bg-danger alert-styled-right', type: 'error'
        });
    });
    $('#pnotify-desktop-danger').on('click', function () {
        ...
        (new PNotify({
            ...
            desktop: { desktop: true, icon: 'assets/images/pnotify/danger.png' }
    ...
    $('#pnotify-desktop-success').on('click', function () {
        ...
        (new PNotify({
            ...
            desktop: { desktop: true, icon: 'assets/images/pnotify/success.png' }
        })).get().click(function (e) {
            if ($('.ui-pnotify-closer, .ui-pnotify-sticker, .ui-pnotify-closer *, .ui-pnotify-sticker *').is(e.target)) return;
            alert('Hey! You clicked the desktop notification!');
        });
    ...
    $('#pnotify-desktop-warning').on('click', function () {
        ...
        (new PNotify({
            ...
            desktop: { desktop: true, icon: 'assets/images/pnotify/warning.png' }
        })).get().click(function (e) {
            if ($('.ui-pnotify-closer, .ui-pnotify-sticker, .ui-pnotify-closer *, .ui-pnotify-sticker *').is(e.target)) return;
            alert('Hey! You clicked the desktop notification!');
        });
    ...
    $('#pnotify-desktop-info').on('click', function () {
        ...
        (new PNotify({
            ...
            desktop: { desktop: true, icon: 'assets/images/pnotify/notice.png' }
        })).get().click(function (e) {
            if ($('.ui-pnotify-closer, .ui-pnotify-sticker, .ui-pnotify-closer *, .ui-pnotify-sticker *').is(e.target)) return;
            alert('Hey! You clicked the desktop notification!');
        });
    ...
    $('#pnotify-rich').on('click', function () {
        new PNotify({
            ...
            text: 'Look at my beautiful <strong>strong</strong>, <a href="#!" class="alert-link">linked</a>, <em>emphasized</em>, and <u>underlined</u> text with <i class="icon-make-group"></i> icon.', icon: 'icon-comment-discussion'
        });
    });
    ...
    $('#pnotify-form').on('click', function () {
        var notice = new PNotify({
            text: $('#form_notice').html(), width: '300px',
            hide: false,
            addclass: 'bg-slate',
            buttons: { closer: false, sticker: false }, insert_brs: false
        }); notice.get().find('button[name=cancel]').on('click', function () { notice.remove(); })
        notice.get().submit(function () {
            var username = $(this).find('input[name=username]').val(); if (!username) { alert('Please provide a username.'); return false; }
            notice.update({ title: 'Welcome', text: 'Successfully logged in as ' + username, addclass: 'bg-teal', icon: true, width: PNotify.prototype.options.width, hide: true, buttons: { closer: true, sticker: true } }); return false;
        });
    });
    ...
    $('#pnotify-confirm').on('click', function () {
        var notice = new PNotify({
            ...
            confirm: {
                ...
                buttons: [{
                    text: 'Yes',
                    addClass: 'btn btn-sm btn-primary'}, {addClass: 'btn btn-sm btn-link'}]
            },
            buttons: { closer: false, sticker: false },
            history: { history: false }
        })
        notice.get().on('pnotify.confirm', function () { alert('Ok, cool.'); })
        notice.get().on('pnotify.cancel', function () { alert('Oh ok. Chicken, I see.'); });
    });
    ...
    $('#pnotify-buttons').on('click', function () {
        new PNotify({
            ...
            buttons: { closer: false, sticker: false },
            history: { closer: 'icofont icofont-close-circled', history: false }
        });
    });
                    ...
                    before_close: function (PNotify) {
                        PNotify.update({
                            ...
                            before_close: function (PNotify) {
                                ...
                                PNotify.queueRemove(); return false;
                            }
                        });
                        PNotify.queueRemove(); return false;
                    }
    ...
    function show_stack_top_left(type) {
        ...
        switch (type) {
            case 'error': opts.title = "Oh No";
                opts.text = "Watch out for that water tower!";
                opts.addclass = "stack-top-left bg-danger";
                opts.type = "error"; break;
            case 'info': opts.title = "Breaking News";
                opts.text = "Have you met Ted?";
                opts.addclass = "stack-top-left bg-info";
                opts.type = "info"; break;
            case 'success': opts.title = "Good News Everyone";
                opts.text = "I've invented a device that bites shiny metal asses.";
                opts.addclass = "stack-top-left bg-success";
                opts.type = "success"; break;
        }
        new PNotify(opts);
    }
    ...
    function show_stack_custom_bottom(type) {
        ...
        switch (type) {
            case 'error': opts.title = "Oh No";
                ...
                opts.type = "error"; break; case 'info': opts.title = "Breaking News";
                opts.text = "Have you met Ted?";
